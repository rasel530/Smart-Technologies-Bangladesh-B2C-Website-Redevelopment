...
export default function PhoneVerificationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response: VerificationResponse = await fetch('/api/v1/auth/verify-phone', {
        method: 'POST',
        headers: {
  ...
  const handleResendCode = async () => {
    ...
    try {
      const response = await fetch('/api/v1/auth/send-phone-verification', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="text-center">
          ...
          <h2 className="text-3xl font-bold text-gray-900">
            {language === 'bn' ? 'ফোন ভেরিফিকেশন' : 'Phone Verification'}
          </h2>
          <p className="mt-2 text-sm text-gray-600">
            {language === 'bn' 
              ? `আপনার ফোন (${phone}) এ পাঠানো ভেরিফিকেশন কোডটি লিখুন`
          ...
          <div>
            ...
            >
              {verificationStatus === 'success' ? (
                ...
                <span>
                  {language === 'bn' ? 'ভেরিফিকেশন করুন' : 'Verify Phone'}
                </span>
              )}
            </button>
          </div>
