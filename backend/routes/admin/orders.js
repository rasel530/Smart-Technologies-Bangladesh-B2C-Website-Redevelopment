...
router.get('/sharing', [
  ...
  try {
    ...
    const orderIds = shares.map(share => share.order_id);
    ...
    const mappedShares = shares.map(share => ({
      ...
      lastAccessedAt: share.last_viewed_at?.toISOString() || null,
...
router.get('/sharing/stats', authMiddleware.authenticate(), authMiddleware.adminOnly(), async (req, res) => {
  try {
    ...
    const byTypeMap = byType.reduce((acc, item) => {
      acc[item.share_type] = item._count.share_type;
      return acc;
    }, {
      public_link: 0,
      protected_link: 0,
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const dayViews = dayShares.reduce((sum, share) => sum + share.view_count, 0);
