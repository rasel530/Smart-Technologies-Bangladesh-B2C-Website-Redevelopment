// ============================================
// RBAC Schema Investigation Script
// ============================================
...
async function investigateRBAC() {
  ...
  try {
    ...
    if (testUserResult.rows.length > 0) {
      ...
      console.log(`  Clearance Level: ${testUser.clearance_level}`);
      console.log(`  Created: ${testUser.created_at}`);
      console.log();

      // Get user's RBAC roles
      const userRBACRolesResult = await client.query(`
    ...
    rolesWithCartPermsResult.forEach(rp => {
    ...
    // Check roles table constraints
    const rolesConstraintsResult = await client.query(`
      SELECT
        tc.constraint_name,
        tc.constraint_type,
        kcu.column_name,
    ...
    console.log('Roles table constraints:');
    rolesConstraintsResult.rows.forEach(c => {
    ...
    if (hasPermissionFuncResult.rows[0].exists) {
      ...
      if (testUserResult.rows.length > 0) {
        ...
        console.log(`\nTesting permissions for user ${testUserResult.rows[0].email}:`);
    ...
    if (testUserResult.rows.length > 0) {
      ...
      console.log(`  - Email: ${testUserResult.rows[0].email}`);
      console.log(`  - Legacy Role: ${testUserResult.rows[0].role}`);
      console.log(`  - Clearance Level: ${testUserResult.rows[0].clearance_level}`);
      console.log(`  - RBAC Roles Assigned: ${userRBACRolesResult.rows.length}`);
      console.log(`  - Total Permissions: ${userPermsResult.rows.length}`);
      console.log();
    }
