/**
 * Test UUID Regex Fix for EMI Deletion - Final Version
 * 
 ...
 * 
 * Uses existing EMI providers and plans from the database for testing.
 */
...
function testDeletePlanByCode(token, planCode) {
  const testName = 'Test 1: Delete EMI Plan by Code (was working before)';
  const response = curlRequest('DELETE', `/api/v1/admin/emi/plans/${planCode}`,
...
function testDeletePlanByUUID(token, planId) {
  const testName = 'Test 2: Delete EMI Plan by UUID (was failing before due to regex bug)';
  const response = curlRequest('DELETE', `/api/v1/admin/emi/plans/${planId}`,
    { 'Authorization': `Bearer ${token}` }
  );
...
function testDeleteProviderByCode(token, providerCode) {
  const testName = 'Test 4: Delete EMI Provider by Code (was working before)';
  const response = curlRequest('DELETE', `/api/v1/admin/emi/providers/${providerCode}`,
    { 'Authorization': `Bearer ${token}` }
  );
...
function testDeleteProviderByUUID(token, providerId) {
  const testName = 'Test 5: Delete EMI Provider by UUID (was failing before due to regex bug)';
  const response = curlRequest('DELETE', `/api/v1/admin/emi/providers/${providerId}`,
    { 'Authorization': `Bearer ${token}` }
  );
...
function runTests() {
  ...
  // Step 3: Get existing EMI data
  console.log('\n--- Step 3: Using Existing EMI Data ---');
  // Use existing provider and plan from database
  const provider = {
    id: '73d6b2dc-fcb4-4017-80ab-3539c0dd4003',
    code: 'city-bank-emi-1',
    name: 'City Bank EMI'
  };
  
  const plan = {
    id: '337cabf9-d462-4f0f-b14b-428e812641fe',
    code: 'city-bank-emi-1-3m-1',
    name: '3 Months EMI'
  };
  
  console.log(`✓ Using existing provider: ${provider.name} (ID: ${provider.id}, Code: ${provider.code})`);
  console.log(`✓ Using existing plan: ${plan.name} (ID: ${plan.id}, Code: ${plan.code})`);
  recordTest('Use Existing Data', true, { provider, plan });

  // Step 4: Test EMI Plan Deletion
  console.log('\n--- Step 4: Testing EMI Plan Deletion ---');
  testDeletePlanByCode(token, plan.code);
  testDeletePlanByUUID(token, plan.id);
  testDeletePlanInvalidUUID(token);

  // Step 5: Test EMI Provider Deletion
  console.log('\n--- Step 5: Testing EMI Provider Deletion ---');
  testDeleteProviderByCode(token, provider.code);
  testDeleteProviderByUUID(token, provider.id);
