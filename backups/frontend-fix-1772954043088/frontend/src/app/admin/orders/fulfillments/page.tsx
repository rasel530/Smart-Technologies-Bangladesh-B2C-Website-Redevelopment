...
export default function AdminOrderFulfillmentsPage() {
  ...
  const filteredFulfillments = fulfillments.filter(fulfill => {
    if (courierFilter !== 'all' && fulfill.courierServiceId !== courierFilter) return false;
    if (searchQuery && !fulfill.orderId.toLowerCase().includes(searchQuery.toLowerCase()) && 
        !fulfill.trackingNumber?.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });
