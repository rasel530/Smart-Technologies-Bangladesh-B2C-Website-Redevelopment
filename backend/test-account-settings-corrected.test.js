 ...
 * 
 * Run with: node test-account-settings-corrected.test.js
 */
...
async function testNotificationSettingsPersistence(jwtToken) {
  ...
  try {
    ...
    if (response.statusCode === 200 && response.body) {
      logTest('Notification Settings - Get Initial', 'PASSED', {
        ...
        initialSettings: response.body.data
      });
    } else {
  ...
  try {
    ...
    if (response.statusCode === 200 && response.body) {
      logTest('Notification Settings - Save', 'PASSED', {
        ...
        savedSettings: response.body.data
      });
    } else {
  ...
  try {
    ...
    if (response.statusCode === 200 && response.body) {
      const retrievedSettings = response.body.data;
      const settingsMatch = 
        retrievedSettings?.email === newSettings.email &&
        retrievedSettings?.sms === newSettings.sms &&
        retrievedSettings?.whatsapp === newSettings.whatsapp &&
        retrievedSettings?.push === newSettings.push;
