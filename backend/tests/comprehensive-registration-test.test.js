...
const testValidRegistrationScenarios = async () => {
    ...
    // Test 3: Property name compatibility (firstName/fname)
    try {
        const email = generateTestEmail();
        const password = generateStrongPassword();
        const result = await measureResponseTime(async () => {
            return await makeRequest('POST', '/auth/register', {
                ...
                password,
                confirmPassword: password,
                fname: 'Test', // Using fname instead of firstName
                lname: 'User'  // Using lname instead of lastName
            });
        });

        if (result.success && result.status === 201) {
            logTest('Property name compatibility (fname/lname)', 'PASS', {
                email,
                responseTime: result.responseTime,
                userId: result.data.user?.id
            });
        } else {
            logTest('Property name compatibility (fname/lname)', 'FAIL', {
                email,
                status: result.status,
                error: result.data?.error || 'Unknown error',
                responseTime: result.responseTime
