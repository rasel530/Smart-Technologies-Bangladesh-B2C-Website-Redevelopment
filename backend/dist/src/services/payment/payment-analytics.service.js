...
exports.paymentAnalyticsService = exports.PaymentAnalyticsService = void 0;
...
class PaymentAnalyticsService {
     ...
     * @returns Created analytics record
     */
    async aggregateMonthlyAnalytics(year, month) {
        try {
     ...
     */
    async getAnalyticsByDateRange(startDate, endDate, period) {
        try {
     ...
     */
    async getGatewayAnalytics(gateway, startDate, endDate) {
        try {
            const transactions = await prisma.payment_transaction.findMany({
                where: {
     ...
     */
    async getMethodAnalytics(method, startDate, endDate) {
        try {
            const transactions = await prisma.payment_transaction.findMany({
                where: {
    ...
    async getFailureAnalysis(startDate, endDate) {
        try {
            ...
            const failuresByReasonMap = new Map();
            ...
            const failuresByPaymentMethodMap = new Map();
            ...
            const failuresByGatewayMap = new Map();
            for (const transaction of transactions) {
     ...
     */
    async getRevenueTracking(startDate, endDate) {
        try {
            const transactions = await prisma.payment_transaction.findMany({
                where: {
                    createdAt: {
            ...
            // Revenue by gateway
            const revenueByGatewayMap = new Map();
            for (const transaction of transactions) {
                if (transaction.status === 'completed') {
                    const gateway = this.getGatewayForPaymentMethod(transaction.paymentMethod);
    ...
    async getPerformanceMetrics(startDate, endDate) {
        try {
            ...
            const p99ResponseTime = responseTimes.length > 0
                ...
                : 0;
            // Gateway performance
            const gatewayPerformanceMap = new Map();
            for (const transaction of transactions) {
                const gateway = this.getGatewayForPaymentMethod(transaction.paymentMethod);
            ...
            const paymentMethodPerformanceMap = new Map();
            for (const transaction of transactions) {
                const method = transaction.paymentMethod;
     ...
     */
    calculateGatewayBreakdown(transactions) {
     ...
     */
    calculateMethodBreakdown(transactions) {
        const breakdown = new Map();
        for (const transaction of transactions) {
            const method = transaction.paymentMethod;
     ...
     */
    calculateRevenueForecast(dailyRevenue) {
...
exports.paymentAnalyticsService = new PaymentAnalyticsService();
