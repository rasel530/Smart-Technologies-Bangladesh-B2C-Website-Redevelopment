...
async function testDatabaseSchema() {
  ...
  try {
    ...
    // Test 1.5: Check if users table has accountStatus column
    console.log('Testing users table for accountStatus column...');
    try {
      const result = await prisma.$queryRaw`
        SELECT column_name FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'accountStatus'
      `;
      logTest('database', 'users table has accountStatus column', result.length > 0);
    } catch (error) {
      logTest('database', 'users table has accountStatus column', false, error);
    }

    // Test 1.6: Check if users table has deletionRequestedAt column
    console.log('Testing users table for deletionRequestedAt column...');
    try {
      const result = await prisma.$queryRaw`
        SELECT column_name FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'deletionRequestedAt'
      `;
      logTest('database', 'users table has deletionRequestedAt column', result.length > 0);
    } catch (error) {
      logTest('database', 'users table has deletionRequestedAt column', false, error);
    }

    // Test 1.7: Check if users table has deletedAt column
    console.log('Testing users table for deletedAt column...');
    try {
      const result = await prisma.$queryRaw`
        SELECT column_name FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'deletedAt'
      `;
      logTest('database', 'users table has deletedAt column', result.length > 0);
    } catch (error) {
      logTest('database', 'users table has deletedAt column', false, error);
    }

    // Test 1.8: Check if users table has deletionReason column
    console.log('Testing users table for deletionReason column...');
    try {
      const result = await prisma.$queryRaw`
        SELECT column_name FROM information_schema.columns 
        WHERE table_name = 'users' AND column_name = 'deletionReason'
      `;
      logTest('database', 'users table has deletionReason column', result.length > 0);
    } catch (error) {
      logTest('database', 'users table has deletionReason column', false, error);
    }
