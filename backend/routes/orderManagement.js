...
// Diagnostic logging to check if order_modifications model exists
console.log('[OrderManagement] Prisma client models available:', Object.keys(prisma).filter(key => !key.startsWith('$') && !key.startsWith('_')));
console.log('[OrderManagement] order_modifications model exists:', !!prisma.order_modifications);
console.log('[OrderManagement] order_modifications type:', typeof prisma.order_modifications);
...
router.get('/admin/orders/reports', [
  ...
  try {
    ...
    switch (reportType) {
      ...
      case 'modifications':
        // Build WHERE clause dynamically for raw query
        let whereClause = '';
        const params = [];
        if (where.created_at) {
          ...
          if (where.created_at.lte) {
            whereClause += (whereClause ? ' AND' : ' WHERE') + ' created_at <= $' + (params.length + 1);
            params.push(where.created_at.lte);
          }
        }

        // Get modifications using raw query
        const modifications = await prisma.$queryRawUnsafe(
          `SELECT modification_type, status, created_at FROM order_modifications${whereClause} ORDER BY created_at DESC`,
          ...params
        );
...
router.get('/admin/orders/analytics', [
  ...
  try {
    ...
    // Get modifications using raw query
    const modifications = await prisma.$queryRawUnsafe(
      `SELECT * FROM order_modifications${whereClause}`,
      ...params
    );
...
router.get('/admin/modifications', [
  ...
  try {
    ...
    if (where.modification_type) {
      whereClause += (whereClause ? ' AND' : ' WHERE') + ' modification_type = $' + (params.length + 1);
      params.push(where.modification_type);
    }

    // Get modifications with order details using raw query to bypass Prisma model issue
    const [modifications, totalCount] = await Promise.all([
      prisma.$queryRawUnsafe(
        `SELECT * FROM order_modifications${whereClause} ORDER BY created_at DESC LIMIT $${params.length + 2} OFFSET $${params.length + 3}`,
    ...
    const response = transformPaginatedResponse(
      ...
      { page, limit, totalCount: parseInt(totalCount[0].count) },
...
router.post('/:id/modifications', [
  ...
  try {
    ...
    // Create modification request using raw SQL
    const modificationId = crypto.randomUUID();
    await prisma.$executeRawUnsafe(
      `INSERT INTO order_modifications (id, order_id, modification_type, description, changes, requested_by, status, created_at)
    ...
    console.log(`[Order Management] Modification request created: ${modificationId} for order ${id} by user ${userId}`);
...
router.put('/:id/modifications/:modificationId/approve', [
  ...
  try {
    ...
    // Check if modification exists and belongs to order using raw SQL
    const modifications = await prisma.$queryRawUnsafe(
      `SELECT * FROM order_modifications WHERE id = $1 AND order_id = $2`,
      modificationId,
      id
    );
    ...
    // Update modification status using raw SQL
    await prisma.$executeRawUnsafe(
      `UPDATE order_modifications
       SET status = $1, approved_by = $2, processed_at = NOW()
       WHERE id = $3`,
      'approved',
      adminId,
      modificationId
...
router.get('/:id/modifications', [
  ...
  try {
    ...
    const response = {
      success: true,
      data: formattedModifications.map(mod => transformOrderModification(mod))
    };
    res.json(response);
