 ...
 * Metric type
 */
export type MetricType = 'revenue' | 'transaction_count' | 'success_rate' | 'avg_transaction_value' | 'response_time' | 'failure_rate';
...
export interface KPIs {
  ...
  gatewayResponseTimes: Array<{
    gateway: string;
    averageResponseTime: number;
  }>;
  paymentMethodPopularity: Array<{
    paymentMethod: PaymentMethod;
...
export class PaymentMetricsService {
  ...
  async recordMetric(
    ...
    try {
      ...
      return {
        ...
        paymentMethod: metric.payment_method as PaymentMethod || undefined,
   ...
   * @returns Array of metric records
   */
  async getLatestMetrics(metricName: string, limit: number = 10): Promise<MetricRecord[]> {
    try {
      const metrics = await prisma.payment_metrics.findMany({
        where: {
          metric_name: metricName
   ...
   * @returns KPIs object
   */
  async calculateKPIs(startDate: Date, endDate: Date): Promise<KPIs> {
    try {
   ...
   */
  private async calculateTransactionCountMetrics(
    transactions: any[],
    startDate: Date,
    endDate: Date
  ): Promise<{ hourly: number; daily: number; weekly: number; monthly: number }> {
   ...
   */
  private async calculateSuccessRateMetrics(
    transactions: any[],
    startDate: Date,
    endDate: Date
  ): Promise<{ hourly: number; daily: number; weekly: number; monthly: number }> {
    const calculateSuccessRate = (filteredTransactions: any[]) => {
   ...
   */
  private async calculateAverageTransactionValueMetrics(
    transactions: any[],
    startDate: Date,
    endDate: Date
  ): Promise<{ hourly: number; daily: number; weekly: number; monthly: number }> {
    const calculateAverageTransactionValue = (filteredTransactions: any[]) => {
   ...
   */
  private async calculateGatewayResponseTimes(
    transactions: any[]
  ): Promise<Array<{ gateway: string; averageResponseTime: number }>> {
    ...
    const gatewayResponseTimes: Array<{ gateway: string; averageResponseTime: number }> = [];
    ...
    for (const [gateway, responseTimes] of gatewayMap.entries()) {
      ...
      await this.recordMetric(
        `gateway_response_time_${gateway}`,
   ...
   */
  private async calculatePaymentMethodPopularity(
    transactions: any[]
  ): Promise<Array<{ paymentMethod: PaymentMethod; usagePercentage: number }>> {
    const methodCountMap = new Map<PaymentMethod, number>();
    ...
    const paymentMethodPopularity: Array<{ paymentMethod: PaymentMethod; usagePercentage: number }> = [];
    ...
    for (const [paymentMethod, count] of methodCountMap.entries()) {
      ...
      // Record metric
      await this.recordMetric(
        `payment_method_popularity_${paymentMethod}`,
        usagePercentage,
        'popularity',
        'daily',
        undefined,
  ...
  async getMetricsSummary(): Promise<{
    ...
    try {
      ...
      let topPaymentMethod: PaymentMethod = 'credit_card';
      ...
      // Find top gateway
      const gatewayCountMap = new Map<string, number>();
      for (const transaction of transactions) {
        const gateway = this.getGatewayForPaymentMethod(transaction.paymentMethod);
