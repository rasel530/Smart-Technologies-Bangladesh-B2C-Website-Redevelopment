@echo off
REM ============================================================
REM Weekly Database Backup Script
REM Purpose: Creates weekly backups of PostgreSQL database
REM Schedule: Run every Sunday at 3:00 AM
REM Rotation: Keeps last 4 weeks of backups
REM ============================================================
...
SET BACKUP_DIR=%~dp0weekly
SET LOG_FILE=%~dp0logs\backup-weekly.log
SET TIMESTAMP=%date:~10,4%%date:~4,2%%date:~7,2%
SET BACKUP_FILE=%BACKUP_DIR%\smart_ecommerce_dev_weekly_%TIMESTAMP%.sql
SET BACKUP_FILE_COMPRESSED=%BACKUP_DIR%\smart_ecommerce_dev_weekly_%TIMESTAMP%.sql.gz
...
REM Start logging
call :LOG "=========================================="
call :LOG "Starting Weekly Backup"
call :LOG "=========================================="
...
REM Rotate old backups (keep last 4 weeks)
call :LOG "Rotating old backups (keeping last 4 weeks)..."
for /f "skip=4 delims=" %%F in ('dir /b /o-d "%BACKUP_DIR%\smart_ecommerce_dev_weekly_*.sql.gz"') do (
    call :LOG "Deleting old backup: %%F"
    del "%BACKUP_DIR%\%%F"
)

REM Count remaining backups
for /f %%i in ('dir /b "%BACKUP_DIR%\smart_ecommerce_dev_weekly_*.sql.gz" ^| find /c /v ""') do set BACKUP_COUNT=%%i
call :LOG "Current backup count: %BACKUP_COUNT%"

REM List all backups
call :LOG "Current backups in directory:"
dir /b "%BACKUP_DIR%\smart_ecommerce_dev_weekly_*.sql.gz" >> "%LOG_FILE%"
...
call :LOG "=========================================="
call :LOG "Weekly Backup Completed Successfully"
call :LOG "=========================================="
