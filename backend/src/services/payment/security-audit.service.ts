...
export class SecurityAuditService {
  ...
  async logSecurityEvent(
    ...
    try {
      ...
      throw new PaymentGatewayError(
        ...
        'SECURITY_AUDIT_FAILED' as PaymentErrorType,
        500
      );
    }
  }
   ...
   */
  async getSecurityAuditLogs(filters: AuditFilters): Promise<SecurityAudit[]> {
    try {
      const where: any = {};
      ...
      throw new PaymentGatewayError(
        'Failed to fetch security audit logs',
        'SECURITY_AUDIT_FAILED' as PaymentErrorType,
        500
      );
    }
   ...
   */
  async getSecurityAuditById(id: string): Promise<SecurityAudit> {
    try {
      ...
      if (!audit) {
        throw new PaymentGatewayError(
          ...
          'SECURITY_AUDIT_NOT_FOUND' as PaymentErrorType,
          404
        );
      }
  ...
  async getSecurityStatistics(
    ...
    try {
      ...
      const statistics: SecurityStats = {
        totalEvents,
        eventsBySeverity,
        eventsByType,
        resolvedEvents,
  ...
  async getCriticalSecurityEvents(
    ...
    try {
      ...
      throw new PaymentGatewayError(
        'Failed to fetch critical security events',
        'SECURITY_AUDIT_FAILED' as PaymentErrorType,
        500
      );
    }
   ...
   */
  async getSecurityTrends(period: string): Promise<SecurityTrends> {
    try {
      let startDate: Date;
      let groupBy: string;
      ...
      const data: SecurityTrends['data'] = [];
      const now = new Date();
      ...
      throw new PaymentGatewayError(
        'Failed to fetch security trends',
        'SECURITY_AUDIT_FAILED' as PaymentErrorType,
        500
      );
    }
   ...
   */
  async exportSecurityAuditLogs(filters: AuditFilters): Promise<Buffer> {
    try {
      ...
      throw new PaymentGatewayError(
        'Failed to export security audit logs',
        'SECURITY_AUDIT_FAILED' as PaymentErrorType,
        500
      );
    }
